...
namespace HG\ImageRepositoryBundle\Adapter;
 ...
 * @version SVN: $Id$
 */
class AdapterGD extends AdapterAbstract
{
  ...
  */
  ...
  public function __construct()
  {
    ...
    {
      throw new ImageTransformException('The image processing library GD is not enabled. See PHP Manual for installation instructions.');
    }
  }
  ...
  public function load($filename, $mime)
  {
    ...
    {
        throw new ImageTransformException(sprintf('Cannot load file %s as %s is an unsupported file type.', $filename, $mime));
    }
  }
  ...
  public function saveAs($filename, $mime='')
  {
    if ('' !== $mime)
    {
      ...
      {
        throw new ImageTransformException(sprintf('Cannot convert as %s is an unsupported type' ,$mime));
      }
    }
 ...
 /**
  ...
  public function getColorByHex($image, $color)
  {
    ...
    {
      if (preg_match('/#[\d\w]{8}/',$color))
      {
        $rgb = sscanf($color, '#%2x%2x%2x%2x');
        $color = imagecolorallocatealpha($image, $rgb[0], $rgb[1], $rgb[2], $rgb[3]);        
      }
      else if (preg_match('/#[\d\w]{6}/',$color))
      {
